...
public class UserDaoJdbc implements UserDao {
  ...
  public User add(User user) {
    int generatedId = this.userInsert
        .executeAndReturnKey(new UserBeanPropertySqlParameterSource(user)).intValue();
  ...
  public User update(User user) {
    int affected = jdbcTemplate.update(
        "update users set " + "name = :name, " + "username = :username, " + "password = :password, "
            + "type = :type, " + "groupid = :groupid, " + "created = :created, "
            + "modified = :modified, " + "logins = :logins " + "where id = :id",
  ...
  public User get(int id) {
    try {
      return this.jdbcTemplate.queryForObject("select * from users where id = ?", this.rowMapper,
          id);
  ...
  private static class UserBeanPropertySqlParameterSource
      extends MappedBeanPropertySqlParameterSource {
